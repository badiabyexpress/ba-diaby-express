        ...
        <React.Fragment key={s.key}>
          <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); onStepClick?.(i); }} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, background: "none", border: "none", padding: "6px 8px", margin: "-6px -8px", cursor: onStepClick ? "pointer" : "default", WebkitTapHighlightColor: "transparent", touchAction: "manipulation" }}>
            <div style={{
              ...
              color: i <= step ? "#fff" : "var(--muted)", fontSize: 12.5, fontWeight: 700, pointerEvents: "none",
...
async function downloadInvoice(colis, data, options = {}) {
  ...
  // Cette ligne partage sa zone avec le filet séparateur qui commence juste en dessous
  // (Z.parties - 6) : un caractère trop grand ou une position trop basse fait passer le filet en
  // plein milieu du texte, comme constaté avec "ENREGISTRÉ PAR" à la taille précédente (9 pt).
  // Remontée et réduite pour laisser une vraie marge avant le filet.
  if (colis.referenceCommande) {
